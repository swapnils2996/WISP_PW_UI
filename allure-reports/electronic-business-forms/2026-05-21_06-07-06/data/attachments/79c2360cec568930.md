...
- Name: electronicBusinessForms.spec.ts >> Electronic Business Forms >> EBF_AUR_WTC04 - Remove From List without selection - no row deleted
- Location: tests/electronicBusinessForms.spec.ts:128:7
...
Call log:
  - waiting for locator('app-alarm-update-report').locator('input[value="Remove From List"]').first()
